...
export class UpdateShortChatMessage extends Update {
  ...
  async init(update: Api.UpdateShortChatMessage, TelestaticClient: Telestatic) {
    ...
    if (update.fromId) {
      let from = new From();
      if (!update.out) {
        await from.init(BigInt(toString(update.fromId!) as string), TelestaticClient);
      } else {
    ...
    if (update.chatId) {
      let chat = new Chat();
      await chat.init(BigInt(toString(update.chatId!) as string), TelestaticClient);
