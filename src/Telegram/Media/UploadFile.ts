...
export async function UploadFile(
  ...
  try {
    telestaticClient.log.debug('Running telegram.uploadFile');
    if (more?.workers !== undefined) {
      if (!inRange(more?.workers!, 1, 16)) {
        telestaticClient.log.warning(
          `Workers (${more.workers}) out of range (1 <= workers <= 16). Chances are this will make telestatic unstable.`
        );
      }
    }
    if (Buffer.isBuffer(file)) {
      ...
      return telestaticClient.client.uploadFile({
        file: toUpload,
      ...
      if (/^http/i.exec(file)) {
        ...
        return await telestaticClient.client.uploadFile({
          file: toUpload,
          workers: more?.workers || 1,
          onProgress: more?.onProgress,
        });
    ...
    telestaticClient.log.error('Failed to running telegram.uploadFile');
    throw new BotError(
      error.message,
      'telegram.uploadFile',
