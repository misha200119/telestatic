...
export interface sendVideoNoteMoreParams extends sendMediaMoreParams {
  workers?: number;
  mimeType?: string;
...
function clean(more?: sendVideoNoteMoreParams) {
  if (more) {
    let purge = [
      'workers',
      'mimeType',
...
export async function SendVideoNote(
  ...
  fileId: string | Buffer | Medias.MediaVideoNote,
  more?: sendVideoNoteMoreParams
) {
  try {
    ...
    switch (fileId.constructor) {
      ...
      case String:
        ...
        if (/^http/i.test(String(fileId)) || /^(\/|\.\.?\/|~\/)/i.test(String(fileId))) {
          ...
          if (file.typeId !== 13) throw new Error(`Invalid fileId. This fileId not for sticker`);
          return await SendMedia(
            telestaticClient,
            chatId,
            new Api.InputMediaDocument({
    ...
    telestaticClient.log.error('Failed to running telegram.sendVideoNote');
    throw new BotError(
      error.message,
