...
export class ResultGetEntity {
  ...
  noforward?: boolean;
  constructor() {}
  async init(resultsGetEntity: Api.TypeUser | Api.TypeChat | Api.Channel, telestaticClient: Telestatic) {
    ...
    if (resultsGetEntity instanceof Api.User) {
      ...
      if (resultsGetEntity.photo instanceof Api.UserProfilePhoto) {
        ...
        await this.photo.encode(resultsGetEntity.photo, this.id!, this.accessHash!, telestaticClient);
...
export async function GetEntity(
  ...
  try {
    telestaticClient.log.debug('Running telegram.getEntity');
    if (useCache) {
      if (typeof chatId == 'number') {
        if (telestaticClient.entityCache.get(BigInt(toString(chatId) as string) as bigint)) {
          telestaticClient.log.debug(
            'Entity already in cache, so we use it, not fetching fresh data from telegram.'
          );
          //@ts-ignore
          return telestaticClient.entityCache.get(BigInt(toString(chatId) as string) as bigint);
        }
      }
      if (typeof chatId == 'bigint') {
        if (telestaticClient.entityCache.get(chatId as bigint)) {
          telestaticClient.log.debug(
            'Entity already in cache, so we use it, not fetching fresh data from telegram.'
          );
          //@ts-ignore
          return telestaticClient.entityCache.get(chatId as bigint);
        }
      }
      if (typeof chatId == 'string') {
        if (String(chatId).startsWith('@')) {
          if (telestaticClient.entityCache.get(String(chatId).replace('@', '') as string)) {
            telestaticClient.log.debug(
              'Entity already in cache, so we use it, not fetching fresh data from telegram.'
            );
            //@ts-ignore
            return telestaticClient.entityCache.get(String(chatId).replace('@', '') as string);
          }
        }
      }
    }
    ...
    let e = await telestaticClient.client.getEntity(convertId(chatId));
    telestaticClient.log.debug('Creating ResultGetEntity');
    let r = new ResultGetEntity();
    await r.init(e, telestaticClient);
    telestaticClient.log.debug('Caching ResultGetEntity');
    telestaticClient.entityCache.set(r.id, r);
    if (r.username) telestaticClient.entityCache.set(r.username, r);
    return r;
  } catch (error: any) {
    telestaticClient.log.error('Failed running telegram.getEntity');
