...
export class MessageContext extends Message {
  ...
  async reply(text: string, more?: replyMoreParams) {
    if (this.id && this.chat.id) {
      let client: Telestatic = this.TelestaticClient;
  ...
  async delete() {
    let client = this.TelestaticClient;
  ...
  async click({
    ...
    const buildPeer = () => {
      if (this.chat.id === this.TelestaticClient.aboutMe.id) {
    ...
    const buildPeerBot = () => {
      if (this.from.bot) {
        if (this.from.id === this.TelestaticClient.aboutMe.id) {
          return new Api.InputPeerSelf();
        }
        return new Api.InputPeerUser({
      ...
      if (this.replyMarkup?.inlineKeyboard) {
        ...
        if (keyboard && keyboard.callbackData) {
          ...
          if (password !== undefined) {
            let pwd = await this.TelestaticClient.client.invoke(new Api.account.GetPassword());
          ...
          return await this.TelestaticClient.client.invoke(request);
        }
          ...
          if (keyboard.requestContact) {
            if (
              ...
              return this.telegram.sendContact(
                ...
                {
                  phoneNumber:
                    (sharePhone === true ? this.TelestaticClient.aboutMe.phone : sharePhone) ?? '',
                  firstName: this.TelestaticClient.aboutMe.firstName ?? 'unknown',
                  lastName: this.TelestaticClient.aboutMe.lastName ?? '',
                  vcard: '',
                },
                {
                  replyToMsgId: this.id,
