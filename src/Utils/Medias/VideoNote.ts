...
export class MediaVideoNote extends Media {
  ...
  async encode(videoNote: Api.Document, telestaticClient: Telestatic) {
    telestaticClient.log.debug('Creating MediaVideoNote');
  ...
  async download(fileId?: string, params?: DownloadFileParams) {
    this.telestaticClient.log.debug('Downloading VideoNote');
    ...
    if (!file) {
      this.telestaticClient.log.error('Failed to download VideoNote cause: FileId not found!');
      throw new BotError('FileId not found!', 'VideoNote.download', String(file));
    }
    if (file !== this.fileId) {
      let dFile = this.decode(file);
      if (dFile.typeId !== 13) {
        this.telestaticClient.log.error('Failed to download videoNote cause: Miss match file type!');
