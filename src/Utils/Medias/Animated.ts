...
export class MediaAnimation extends Media {
  ...
  async encode(animated: Api.Document, telestaticClient: Telestatic) {
    telestaticClient.log.debug('Creating MediaAnimation');
  ...
  async download(fileId?: string, params?: DownloadFileParams) {
    this.telestaticClient.log.debug('Downloading animation');
    ...
    if (!file) {
      this.telestaticClient.log.error('Failed to download animation cause: FileId not found!');
      throw new BotError('FileId not found!', 'Animation.download', String(file));
    }
    if (file !== this.fileId) {
      let dFile = this.decode(file);
      if (dFile.typeId !== 4) {
        this.telestaticClient.log.error('Failed to download animation cause: Miss match file type!');
