...
export class MediaPhoto extends Media {
  ...
  async encode(media: Api.MessageMediaPhoto | Api.Photo | Api.PhotoEmpty, telestaticClient: Telestatic) {
    telestaticClient.log.debug('Creating MediaPhoto');
  ...
  async download(fileId?: string, params?: DownloadFileParams) {
    this.telestaticClient.log.debug('Downloading photo');
    ...
    if (!file) {
      this.telestaticClient.log.error('Failed to download photo cause: FileId not found!');
      throw new BotError('FileId not found!', 'Photo.download', String(file));
    }
    if (file !== this.fileId) {
      let dFile = this.decode(file);
      if (dFile.typeId !== 2) {
        this.telestaticClient.log.error('Failed to download photo cause: Miss match file type!');
