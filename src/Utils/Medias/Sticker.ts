...
export class MediaSticker extends Media {
  ...
  async encode(sticker: Api.Document, telestaticClient: Telestatic) {
    telestaticClient.log.debug('Creating MediaSticker');
  ...
  async download(fileId?: string, params?: DownloadFileParams) {
    this.telestaticClient.log.debug('Downloading sticker');
    ...
    if (!file) {
      this.telestaticClient.log.error('Failed to download sticker cause: FileId not found!');
      throw new BotError('FileId not found!', 'Sticker.download', String(file));
    }
    if (file !== this.fileId) {
      let dFile = this.decode(file);
      if (dFile.typeId !== 8) {
        this.telestaticClient.log.error('Failed to download sticker cause: Miss match file type!');
