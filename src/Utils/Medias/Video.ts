...
export class MediaVideo extends Media {
  ...
  async encode(video: Api.Document, telestaticClient: Telestatic) {
    telestaticClient.log.debug('Creating MediaVideo');
  ...
  async download(fileId?: string, params?: DownloadFileParams) {
    this.telestaticClient.log.debug('Downloading video');
    ...
    if (!file) {
      this.telestaticClient.log.error('Failed to download video cause: FileId not found!');
      throw new BotError('FileId not found!', 'Video.download', String(file));
    }
    if (file !== this.fileId) {
      let dFile = this.decode(file);
      if (dFile.typeId !== 4) {
        this.telestaticClient.log.error('Failed to download video cause: Miss match file type!');
