...
export class MediaDocument extends Media {
  ...
  async encode(document: Api.Document, telestaticClient: Telestatic) {
    telestaticClient.log.debug('Creating MediaDocument');
  ...
  async download(fileId?: string, params?: DownloadFileParams) {
    this.telestaticClient.log.debug('Downloading Document');
    ...
    if (!file) {
      this.telestaticClient.log.error('Failed to download document cause: FileId not found!');
      throw new BotError('FileId not found!', 'Document.download', String(file));
    }
    if (file !== this.fileId) {
      let dFile = this.decode(file);
      if (dFile.typeId !== 4) {
        this.telestaticClient.log.error('Failed to download document cause: Miss match file type!');
