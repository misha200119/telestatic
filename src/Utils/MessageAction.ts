...
export class MessageAction {
  ...
  emoticon!: string;
  constructor() {}
  async init(messageAction: Api.TypeMessageAction, telestaticClient: Telestatic) {
    telestaticClient.log.debug(`Creating messageAction`);
    if (messageAction instanceof Api.MessageActionChatCreate) {
      ...
      for (let users of messageAction.users) {
        let from = new From();
        await from.init(BigInt(toString(users) as string), telestaticClient);
        c.push(from);
      }
      this.users = c;
      return this;
    ...
    if (messageAction instanceof Api.MessageActionChatEditPhoto) {
      ...
      await media.encode(messageAction.photo!, telestaticClient!);
      this.photo = media;
      return this;
    }
    ...
    if (messageAction instanceof Api.MessageActionChatDeleteUser) {
      ...
      this.user = new From();
      await this.user.init(BigInt(toString(messageAction.userId) as string), telestaticClient);
      return this;
    }
    if (messageAction instanceof Api.MessageActionChatJoinedByLink) {
      ...
      await this.invite.init(BigInt(toString(messageAction.inviterId) as string), telestaticClient);
      return this;
    }
    ...
    if (messageAction instanceof Api.MessageActionChannelMigrateFrom) {
      ...
      this.title = messageAction.title;
      this.chat = new Chat();
      await this.chat.init(BigInt(`-${toString(messageAction.chatId)}` as string), telestaticClient);
      return this;
    }
